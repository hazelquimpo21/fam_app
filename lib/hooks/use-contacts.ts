...
/**
 * Import source type for contacts.
 * 
 * POSSIBLE VALUES:
 * - 'manual': User created via ContactModal
 * - 'csv': Imported from CSV file via ImportContactsModal
 * - 'google': Future - imported from Google Contacts API
 */
export type ContactImportSource = 'manual' | 'csv' | 'google';
 ...
 * 
 * IMPORT TRACKING:
 * - imported_from: Tracks how the contact was created (manual/csv/google)
 * - If not provided, defaults to 'manual'
 */
export interface CreateContactInput {
  ...
  country?: string;
  /** 
   * Optional: Source of the contact import.
   * Defaults to 'manual' if not specified.
   * Set to 'csv' when importing from CSV files.
   */
 ...
 *
 * FLOW:
 * 1. Fetch current user's family_id and member_id (required for RLS)
 * 2. Insert new row in contacts table with family_id
 * 3. Set imported_from to 'manual' (user-created)
 * 4. On success: invalidate all contact queries, show toast
 * 5. On error: show error toast
 *
 * IMPORTANT - RLS REQUIREMENT:
 * The contacts table has Row Level Security (RLS) enabled with policy:
 *   "Manage contacts" ON contacts FOR ALL USING (family_id = get_my_family_id())
 * 
 * For INSERT operations, the family_id MUST be explicitly provided in the
 * insert data. The RLS policy checks that the provided family_id matches
 * the user's family. Without family_id, the insert will fail with:
 *   "new row violates row-level security policy for table contacts"
 *
 * AUTO-SET FIELDS:
 * - id: Generated by Supabase (UUID)
 * - family_id: Fetched from family_members table (REQUIRED for RLS)
 * - created_by: Current user's family_member id
 * - imported_from: 'manual'
 * - created_at, updated_at: Set by database defaults
 *
 * @example
...
export function useCreateContact() {
  ...
  return useMutation({
    mutationFn: async (input: CreateContactInput) => {
      ...
      // The RLS policy on contacts checks: family_id = get_my_family_id()
      // Without providing family_id, the insert fails with RLS violation.
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      const { data: { user } } = await supabase.auth.getUser();

      if (!user) {
        logger.error('❌ No authenticated user - cannot create contact');
        throw new Error('You must be logged in to add contacts');
      }

      logger.debug('🔐 Authenticated user found', { userId: user.id });

      // Fetch the user's family membership to get family_id
      ...
      if (memberError) {
        logger.error('❌ Failed to get family info', { 
      ...
      if (!member) {
        logger.error('❌ User has no family membership', { userId: user.id });
        throw new Error('Please complete onboarding to add contacts');
      }

      logger.debug('👨‍👩‍👧 Family context retrieved', { 
        familyId: member.family_id, 
        memberId: member.id 
      });

      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      // STEP 2: Insert the contact WITH family_id
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      // The family_id is required for the RLS policy check to pass.
      // created_by tracks who created this contact for audit purposes.
      // imported_from defaults to 'manual' but can be 'csv' for imports.
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      
      // Determine import source - defaults to 'manual' if not specified
      const importSource = input.imported_from || 'manual';
      
      // Remove imported_from from input spread to avoid duplication
      const { imported_from: _, ...contactData } = input;
      
      const { data, error } = await supabase
        .from('contacts')
        .insert({
          ...contactData,
          family_id: member.family_id,   // REQUIRED for RLS policy
          created_by: member.id,          // Track who created this contact
          imported_from: importSource,    // Track source: 'manual', 'csv', or 'google'
          imported_at: importSource !== 'manual' ? new Date().toISOString() : null, // Track when imported
        })
        .select()
        .single();
      ...
      logger.success('✅ Contact created!', { 
        name: data?.name, 
        id: data?.id,
        familyId: member.family_id 
      });
